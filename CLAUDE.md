...
- **Never promote a disposable test user to `is_primary_admin` — that row can never be deleted again, by design.** `profiles_guard_delete` blocks deleting any `is_primary_admin = true` row unconditionally, even for service-role, and the UPDATE-side guard equally blocks demoting one back to `false` first. A pre-existing test did this and silently leaked 23 stray undeletable rows into the live database across sessions before it was caught. Test primary-admin guard behaviour against the one real seeded row (`getPrimaryAdminUserId()` in `rls-test-helpers.ts`) instead — safe, since every such assertion expects the operation to fail and leave the row untouched. **Also**: don't bundle a general test-infra fix like this one inside a feature ticket's commit if that ticket is going to be held back from `main` for review — every other commit pushed in between (this session held SPEC-021 back for a security review while shipping SPEC-022 through SPEC-026) then lacks the fix too, so any CI run against those intermediate commits re-triggers the exact bug the fix was for. Four more stray rows leaked this way, caught only because a later e2e test surfaced a fifth "Primary admin" badge that shouldn't have existed. Keep infra fixes as their own standalone commit, ordered before the held-back feature.
- **Playwright discovers/parses every test file across all projects before running any of them.** A module-scope `readFileSync` of a file a `setup` project's own test writes (e.g. a small quiz theme's id, generated at auth-setup time) throws `ENOENT` even though the dependent project correctly waits for `setup` at *run* time — file discovery happens before any execution starts. Read such files lazily, inside the test body/a helper function, not at module scope (SPEC-026).
- **A `vercel.json` `crons` entry with a sub-daily schedule fails the whole deployment outright if the plan tier doesn't support it — and production silently keeps serving the previous deployment.** Added a per-minute (`* * * * *`) cron for the admin-triggered plant-import feature; the push's Vercel commit status went straight to `failure`, linking to Vercel's Cron Jobs pricing docs — this project's plan only supports daily-granularity cron. Nothing else about the deploy was broken (GitHub Actions CI was green), so this was easy to miss without explicitly checking the Vercel commit status separately from CI. Redesigned around a client-driven tick loop (the admin page calls a session-authenticated server action every ~1.5s while a job is in flight) instead of relying on an external scheduler — turned out faster in practice, and removed an entire unauthenticated API surface (a bearer-token-checked route + a proxy.ts carve-out for it) as a side benefit. Always check `gh api repos/<owner>/<repo>/commits/<sha>/status` for the `Vercel` context after a push, not just GitHub Actions CI — they can disagree.
